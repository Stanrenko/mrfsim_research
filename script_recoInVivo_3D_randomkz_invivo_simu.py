...
suffix="_us{}".format(undersampling_factor)
if is_random:
...
filename_kdata_selected = str.split(filename,".dat") [0]+"_kdata{}.npy".format(suffix)

filename_mask= str.split(filename,".dat") [0]+"_mask{}.npy".format(suffix)
...
radial_traj=Radial3D(total_nspokes=nb_allspokes,undersampling_factor=undersampling_factor,npoint=npoint,nb_slices=nb_slices,incoherent=incoherent,mode=mode,is_random=is_random,frac_center=frac_center)
...
traj_all = radial_traj_all.get_traj().reshape(ntimesteps,-1,nb_slices,npoint,3)
traj = radial_traj.get_traj().reshape(ntimesteps,-1,int(nb_slices/undersampling_factor),npoint,3)
traj_selected = np.zeros(traj.shape,dtype=traj.dtype)



all_kz = traj_all[0,0,:,0,2]


kdata_all_channels_all_slices_reshaped=kdata_all_channels_all_slices.reshape(nb_channels,ntimesteps,-1,nb_slices,npoint)
kdata_all_channels_all_slices_selected = np.zeros((nb_channels,)+traj.shape[:-1],dtype=kdata_all_channels_all_slices.dtype)

# t=15
# p=0
# ind_kz = (np.argwhere(all_kz == np.unique(traj[t, :, p, :, 2])[0]))[0][0]


for t in range(ntimesteps):
    for p in range(int(nb_slices/undersampling_factor)):
        ind_kz = (np.argwhere(all_kz == np.unique(traj[t, :, p, :, 2])[0]))[0][0]
        #print(ind_kz)
        kdata_all_channels_all_slices_selected[:,t,:,p,:]=kdata_all_channels_all_slices_reshaped[:,t,:,ind_kz,:]
        traj_selected[ t, :, p, :,:] = traj_all[t, :, ind_kz,:,:]

radial_traj.traj=traj_selected
kdata_all_channels_all_slices_selected=kdata_all_channels_all_slices_selected.reshape((nb_channels,nb_segments,int(nb_slices/undersampling_factor),npoint))
np.save(filename_kdata_selected,kdata_all_channels_all_slices_selected)
#radial_traj.adjust_traj_for_window(window)

nb_segments=radial_traj.get_traj().shape[0]
#
# kdata_all_channels_all_slices_control = np.zeros((nb_channels,nb_segments,int(nb_slices/undersampling_factor),npoint),dtype=kdata_all_channels_all_slices.dtype)
# traj_control = np.zeros((nb_segments,int(nb_slices/undersampling_factor),npoint,3),dtype=traj.dtype)
# traj_all_reshaped = traj_all.reshape((nb_segments,int(nb_slices/1),npoint,3))
#
#
# cursor=0
# mod_samp = 0
# for t in range(nb_segments):
#     print(mod_samp)
#     kdata_all_channels_all_slices_control[:,t,:,:]=kdata_all_channels_all_slices[:,t,mod_samp::undersampling_factor,:]
#     traj_control[t,:,:,:]=traj_all_reshaped[t,mod_samp::undersampling_factor,:,:]
#     cursor+=1
#     if cursor==8:
#         cursor=0
#         mod_samp+=1
#         mod_samp=np.mod(mod_samp,2)
#
# traj_reshaped=traj.reshape((nb_segments,int(nb_slices/undersampling_factor),npoint,3))
#
# #traj_control=traj_control.reshape(175,8,-1,npoint,3)
# np.linalg.norm(traj_control-traj_reshaped)
...
if str.split(filename_b1,"/")[-1] not in os.listdir(folder):
    res = 16
    b1_all_slices=calculate_sensitivity_map_3D(kdata_all_channels_all_slices_selected,radial_traj,res,image_size,useGPU=False,light_memory_usage=light_memory_usage)
    np.save(filename_b1,b1_all_slices)
    del kdata_all_channels_all_slices_selected
    kdata_all_channels_all_slices_selected = np.load(filename_kdata_selected)
...
if str.split(filename_volume,"/")[-1] not in os.listdir(folder):
    volumes_all=simulate_radial_undersampled_images_multi(kdata_all_channels_all_slices_selected,radial_traj,image_size,b1=b1_full,density_adj=False,ntimesteps=ntimesteps,useGPU=False,normalize_kdata=False,memmap_file=None,light_memory_usage=light_memory_usage,normalize_volumes=True)
...
if str.split(filename_mask,"/")[-1] not in os.listdir(folder):
    ...
    mask=build_mask_single_image_multichannel(kdata_all_channels_all_slices_selected,radial_traj,image_size,b1=b1_full,density_adj=False,threshold_factor=1/25, normalize_kdata=False,light_memory_usage=True,selected_spokes=selected_spokes)
...
plt.ion()
for t in range(ntimesteps):
    curr_traj=traj_selected[t]
    plt.scatter(x=curr_traj[:,:,:,0].flatten(),y=curr_traj[:,:,:,1].flatten())
    #plt.clf()
    time.sleep(0.1)
...
for t in range(ntimesteps):
    curr_traj = traj_selected[t]
    plt.scatter(x=curr_traj[:, sl, :, 0].flatten(), y=curr_traj[:, sl, :, 1].flatten(),label=t)
    plt.legend()
    plt.draw()
    plt.pause(0.1)
    plt.clf()
