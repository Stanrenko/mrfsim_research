...
#name = "SquareSimu2D_Dico2Invivo_SNR"
...
df_stats_cf=pd.DataFrame(columns=["NRMSE wT1","NRMSE ff","NRMSE attB1","NRMSE df","SSIM wT1","SSIM ff","SSIM attB1","SSIM df"],index=snr_list)
df_stats_matrix=pd.DataFrame(columns=["NRMSE wT1","NRMSE ff","NRMSE attB1","NRMSE df","SSIM wT1","SSIM ff","SSIM attB1","SSIM df"],index=snr_list)
df_stats_brute=pd.DataFrame(columns=["NRMSE wT1","NRMSE ff","NRMSE attB1","NRMSE df","SSIM wT1","SSIM ff","SSIM attB1","SSIM df"],index=snr_list)

for snr in snr_list:
    print("################## PROCESSING Phantom SNR {} ##################".format(snr))
    filename_paramMap=filename+"_paramMap.pkl"
    filename_paramMask=filename+"_paramMask.npy"
    filename_groundtruth = filename+"_groundtruth.npy"

    filename_kdata = filename+"_kdata.npy"
    filename_mask = filename + "_mask.npy"


    filename_volume = filename+"_volumes_snr{}.npy".format(snr)

    file_map_brute = filename + "_snr{}_brute_MRF_map.pkl".format(snr)
    file_map_matrix = filename + "_snr{}_matrix_MRF_map.pkl".format(snr)
    file_map_cf = filename + "_snr{}_cf_MRF_map.pkl".format(snr)
    ...
    for k in ["wT1","ff","df","attB1"]:
        ...
        df_stats_brute.loc[snr, "NRMSE {}".format(k)] = nrmse_brute
        df_stats_brute.loc[snr, "SSIM {}".format(k)] = ssim_brute
        df_stats_cf.loc[snr, "NRMSE {}".format(k)] = nrmse_cf
        df_stats_cf.loc[snr, "SSIM {}".format(k)] = ssim_cf
        df_stats_matrix.loc[snr, "NRMSE {}".format(k)] = nrmse_matrix
        df_stats_matrix.loc[snr, "SSIM {}".format(k)] = ssim_matrix
...
df_stats_brute.to_pickle("df_stats_brute_snr_{}.pkl".format("numerical"))
df_stats_cf.to_pickle("df_stats_cf_snr_{}.pkl".format("numerical"))
df_stats_matrix.to_pickle("df_stats_cf_clustering_snr_{}.pkl".format("numerical"))
