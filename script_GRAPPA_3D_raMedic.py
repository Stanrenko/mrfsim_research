...
if str.split(filename_save,"/")[-1] not in os.listdir(folder):
    ...
    data = data.reshape((-1, int(nb_all_spokes)) + data.shape[1:])
    data=np.moveaxis(data,2,0)
    data = np.moveaxis(data, 2, 1)

    data_calib = np.array([mdb.data for mdb in data_calib])
    data_calib = data_calib.reshape((-1, int(nb_all_spokes)) + data_calib.shape[1:])
...
lines_to_estimate=all_lines[1::undersampling_factor]
...
radial_traj_calib.traj=radial_traj_all.get_traj().reshape(nb_all_spokes,nb_slices,npoint,3)[:,lines_ref,:,:].reshape(nb_all_spokes,-1,3)
...
for ts in tqdm(range(all_ts)):
    ...
    #kdata_all_channels_for_grappa_calib_padded = np.pad(kdata_all_channels_for_grappa, ((0, 0), pad_y, pad_x), mode="edge")
    #curr_traj_for_grappa_calib_padded = np.pad(curr_traj_for_grappa, (pad_y, pad_x, (0, 0)), mode="edge")

    kdata_all_channels_for_grappa_padded = np.pad(kdata_all_channels_for_grappa, ((0, 0), pad_y, pad_x), mode="constant")
    curr_traj_for_grappa_padded = np.pad(curr_traj_for_grappa, (pad_y, pad_x, (0, 0)), mode="edge")

    kdata_all_channels_for_grappa_calib_padded = np.pad(kdata_all_channels_for_grappa_calib, ((0, 0), pad_y, pad_x), mode="constant")
...
radial_traj_grappa = Radial3D(total_nspokes=all_ts, undersampling_factor=1, npoint=npoint,
