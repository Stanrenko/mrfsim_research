...
if str.split(filename_save_us,"/")[-1] not in os.listdir(folder):
    ...
    data[:, :, lines_measured, :].shape

    data_calib = data[:, :, lines_ref, :]
    data = data[:, :, lines_measured, :]
...
weights=np.zeros((len(calib_lines),nb_channels,nb_channels*len(kernel_x)*len(kernel_y)),dtype=data_calib.dtype)
