...
filename_b1 = str.split(filename,".dat") [0]+"_bart_b1.npy"
...
if str.split(filename_b1,"/")[-1] not in os.listdir(folder):
    res = 16
    b1_all_slices=calculate_sensitivity_map(kdata_all_channels_all_slices,radial_traj,res,image_size,hanning_filter=True,density_adj=True)
...
if str.split(filename_mask, "/")[-1] not in os.listdir(folder):
    mask = build_mask_single_image_multichannel(kdata_all_channels, radial_traj, image_size, b1=b1, density_adj=True,
...
kdata_all_channels=np.load(filename_kdata)[sl]

traj_python=radial_traj.get_traj()
traj_python=np.transpose(traj_python)


traj_python_for_bart=np.zeros((3,)+traj_python.shape[1:],dtype=traj_python.dtype).astype("complex64")
traj_python_for_bart[:2,:,:]=traj_python
traj_python_for_bart=traj_python_for_bart/np.max(traj_python_for_bart)*int(npoint/4)

cfl.writecfl("traj",traj_python_for_bart)

kdata_multi_for_bart_full=kdata_all_channels.reshape(nb_channels,nb_allspokes,npoint).T
kdata_multi_for_bart_full=np.expand_dims(kdata_multi_for_bart_full,axis=0)
cfl.writecfl("kdata_multi_full",kdata_multi_for_bart_full)

coil_img=bart(1,"nufft -i -t traj kdata_multi_full")
cfl.writecfl("coil_img",coil_img)
plot_image_grid(np.moveaxis(np.abs(coil_img).squeeze(),-1,0),nb_row_col=(5,6))
...
#sens=cfl.readcfl("sens")
import os
os.system("bart fft -u $(bart bitmask 0 1) coil_img ksp")
sens=bart(1,"ecalib -m1 ksp")
cfl.writecfl("sens",sens)



plot_image_grid(np.moveaxis(np.abs(sens).squeeze(),-1,0)*np.expand_dims(mask,axis=0),nb_row_col=(5,6))
plot_image_grid(np.abs(b1)*np.expand_dims(mask,axis=0),nb_row_col=(5,6))
...
plot_image_grid(np.moveaxis(np.abs(sens_cc).squeeze(),-1,0)*np.expand_dims(mask,axis=0),nb_row_col=(5,6))
...
kdata_multi_for_bart_reshaped=kdata_multi_for_bart_full.reshape(1,npoint,175,8,nb_channels)
kdata_multi_for_bart_reshaped=np.moveaxis(kdata_multi_for_bart_reshaped,2,-1)
kdata_multi_for_bart_reshaped=np.expand_dims(kdata_multi_for_bart_reshaped,axis=(4,5,6,7,8,9))
cfl.writecfl("kdata_multi_for_bart_reshaped",kdata_multi_for_bart_reshaped)
...
for iter in tqdm(range(1,niter+1)):
    if "basis_used" in bart_command:
        out=np.matmul(out.squeeze(),basis.squeeze().T)
    all_maps_bart,matched_signals = optimizer.search_patterns_test_multi_2_steps_dico(dictfile, np.moveaxis(out.squeeze(),-1,0),retained_timesteps=None)
...
for i in range(niter):
    im=ax[i+1].imshow(makevol(all_maps_bart_all_iter[i][0][k],mask>0),cmap="inferno",vmin=vmin, vmax=vmax)
