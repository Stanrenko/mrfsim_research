...
num=0
inc=1
max_num=max(list(dico_data.keys()))
nb_spokes_between_nav = int(nb_segments/nb_gating_spokes)

for j in range(nb_slices):
    for i in range(nb_allspokes):
        data_combined[:,j,i,:]=dico_data[num][:,j,i,:]
        if i%nb_spokes_between_nav==0:
            print(num)
            num=num+inc
...
for j in range(nb_slices):
    for i in range(nb_gating_spokes):
        data_for_nav_combined[:,j,i,:]=dico_data_nav[num][:,j,i,:]
        num=num+inc
        print(num)
        if num==0 or num==max_num:
            inc=-inc
...
serie_name =str.split(serie,"/")[0]
filename_data_combined_save=folder+"/"+serie_name+".npy"
filename_data_nav_combined_save=folder+"/"+serie_name+"_nav.npy"
filename_seqParams = folder+"/"+serie_name+"_seqParams.pkl"

data_combined=np.moveaxis(data_combined,-2,-3)

np.save(filename_data_combined_save,data_combined)
np.save(filename_data_nav_combined_save,data_for_nav_combined)
