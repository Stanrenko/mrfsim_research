...
def build_and_compile_model_simple(n_outputs):
    model = keras.Sequential([
        layers.Dense(200, activation="elu", name="Dense_1"),
        layers.Dense(100, activation="elu", name="Dense_2"),
        layers.Dense(50, activation="elu", name="Dense_3"),
        layers.Dense(30, activation="elu", name="Dense_4"),
        layers.Dense(30, activation="elu", name="Dense_5"),
...
#
# model_keras=build_and_compile_model_simple()
# callbacks_list = []
...
#     np.transpose(X_TF), np.transpose(Y_TF),batch_size=256,shuffle=True,
#     validation_split=0.2,
#     verbose=1, epochs=1500,callbacks=callbacks_list)
#
# output_scaler.inverse_transform(model_keras.predict(X_TF[:,10].reshape(1,-1)))
# output_scaler.inverse_transform(Y_TF[:,10].reshape(1,-1))
