...
for gr in tqdm(dico_traj_retained.keys()):
    ...
    for sl in tqdm(range(nb_slices)):
        ...
        for j in tqdm(range(n_comp)):
            ...
            plan.set_pts(to_gpu(traj_reco[:, 0]), to_gpu(traj_reco[:, 0]))
...
np.save("all_volumes_singular_gpu_test_weighted.npy",all_volumes_singular_gpu)
...
volumes_all = np.load(filename_volume2Dplus1)
...
return_cost=False
#animate_images(mask)
suffix="_2Dplus1_pca{}".format(n_comp)
if not(load_map):
    #niter = 0
...
dy=1
sl=28+10
for gr in list(range(5)):

    #map_Python = MapFromDict3D("RebuiltMapFromParams_iter{}".format(iter), paramMap=map_for_sim)
    #map_Python.buildParamMap()
    file_mha ="volume_singular_l{}_gr{}".format(l,gr)
    io.write(file_mha,np.abs(all_volumes_singular[gr,l]),tags={"spacing":[dz,dx,dy]})
