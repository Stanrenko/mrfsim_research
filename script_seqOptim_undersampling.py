...
import pickle
with open("res_simul_random_FA_US_v3.pkl","rb") as file:
...
plt.plot(TE_[1:])
res.x
generate_epg_dico_T1MRFSS(r"./mrf_sequence_adjusted_optimized_M0_T1_local_optim_correl_crlb_filter_sp760_optimized_DE_Simu_FF_random_FA_v3.json","./mrf_dictconf_SimReco2.json",FA_,TE_,4,1.87/1000,fileseq_basis="./mrf_sequence_adjusted.json")
...
def cost_function_simul_breaks_breaks_common_KneePhantom(params):
    #global result
    global spokes_count
    ...
    TR_, FA_, TE_ = convert_params_to_sequence_breaks_common(params, min_TR_delay, num_breaks_TE, num_breaks_FA,
                                                             spokes_count,
                                                             inversion)
...
class LogCost(object):
    ...
    def __call__(self,x,**kwargs):
        ...
        with open("x_common_breaks_US_log.pkl", "wb") as file:
            pickle.dump(x, file)
...
log_cost=LogCost(cost_function_simul_breaks_breaks_common_KneePhantom)

res=differential_evolution(cost_function_simul_breaks_breaks_common_KneePhantom,bounds=bounds,callback=log_cost,constraints=constraints,maxiter=1000)#,constraints=constraints)

import pickle
with open("res_simul_common_breaks_US.pkl","wb") as file:
    pickle.dump(res, file)
...
import pickle
with open("res_simul_common_breaks_US.pkl","rb") as file:
    res=pickle.load(file)

cost_function_simul_breaks_breaks_common_KneePhantom(res.x)
...
plt.plot(TE_[1:])
res.x
generate_epg_dico_T1MRFSS(r"./mrf_sequence_adjusted_optimized_M0_T1_local_optim_correl_crlb_filter_sp760_optimized_DE_Simu_FF_US_v1.json","./mrf_dictconf_SimReco2.json",FA_,TE_,3.9,1.87/1000,fileseq_basis="./mrf_sequence_adjusted.json")
