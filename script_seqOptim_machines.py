...
DEFAULT_OPT_CONFIG="seq_opt_config.json"
...
def optimize_sequence_random(optimizer_config):
    ...
    sigma = optimizer_config["sigma"]
    noise_type = optimizer_config["noise_type"]
    noise_size = optimizer_config["noise_size"]

    DFs = optimizer_config["DFs"]
    FFs = optimizer_config["FFs"]
    B1s = optimizer_config["B1s"]
    T1s = optimizer_config["T1s"]

    inversion = optimizer_config["inversion"]
    min_TR_delay = optimizer_config["min_TR_delay"]
    bound_min_TE = optimizer_config["bound_min_TE"]
    bound_max_TE = optimizer_config["bound_max_TE"]
    bound_min_FA = optimizer_config["bound_min_FA"]
    ...
    lambda_FA = optimizer_config["lambda_FA"]
    lambda_time = optimizer_config["lambda_time"]
    lambda_FF = optimizer_config["lambda_FF"]

    lambda_T1 = optimizer_config["lambda_T1"]
    lambda_stdT1 = optimizer_config["lambda_stdT1"]

    maxiter = optimizer_config["maxiter"]

    log_cost = LogCost(cost_function_simul_breaks_random)

    res = differential_evolution(cost_function_simul_breaks_random, bounds=bounds, callback=log_cost,maxiter=maxiter)
    ...
    with open("./optims/log_cost_{}.npy".format(date_time), "w") as file:
        np.save(file,log_cost.cost_values)
