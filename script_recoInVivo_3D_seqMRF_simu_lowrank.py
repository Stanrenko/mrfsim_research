...
grad_J_current=lambda X:grad_J(X,traj,data,npoint,nspoke,nb_slices,scaling=None)
J_current=lambda X:J(X,traj,data,npoint,scaling=None)

grad_J_TV_current=lambda X:grad_J_TV(X,axis,mask=mask)
J_TV_current=lambda X:J_TV(X,axis,mask=mask)

for i in tqdm(range(niter_lowrank)):
    print("############### ITER {} ################".format(i))
    grad=grad_J_current(X)
    grad_reg = grad_J_TV_current(X)
