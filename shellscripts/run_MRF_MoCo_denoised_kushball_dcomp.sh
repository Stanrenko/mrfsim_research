...
# python script_recoInVivo_3D_machines.py coil_compression_bart --filename-kdata $1_no_densadj_kdata.npy --n-comp $NCOMP --dens-adj True --filename-seqParams $1_seqParams.pkl

# echo "######################################################"
# echo "Density compensation calculation"
# python script_recoInVivo_3D_machines.py calculate_dcomp_voronoi_3D --filename-kdata $1_no_densadj_bart${NCOMP}_kdata.npy --filename-seqParams $1_seqParams.pkl --filename-weights $1_weights.npy
# # rm $1_kdata.npy

# # #Rebuild singular volumes for all bins
echo "######################################################"
echo "Rebuilding singular volumes for all bins"
# python script_recoInVivo_3D_machines.py build_volumes_singular_allbins_3D --filename-kdata $1_no_densadj_bart${NCOMP}_kdata.npy --filename-seqParams $1_seqParams.pkl --L0 $NSING --dictfile $2 --useGPU False --filename-b1 $1_no_densadj_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --filename-weights $1_weights.npy --gating-only True --filename-dcomp  $1_bart${NCOMP}_filtered_dcomp.npy #--filename-dcomp  $1_no_densadj_bart${NCOMP}_dcomp.npy
...
# python script_VoxelMorph_machines.py train_voxelmorph --filename-volumes $1_bart${NCOMP}_volume_singular_l0_allbins_gamma_${GAMMASTR}.npy --config-train config_train_voxelmorph_shell.json --nepochs 2000 --kept-bins "0,1,2,3,4" --axis 0 #--lr 0.0002 --init-weights $1_volumes_allbins_denoised_gamma_0_8_vxm_model_weights.h5
