...
python script_recoInVivo_3D_machines.py coil_compression_bart --filename-kdata $1_kdata.npy --n-comp $NCOMP --spoke-start 100 --filename-seqParams $1_seqParams.pkl
...
python script_VoxelMorph_machines.py train_voxelmorph --filename-volumes $1_bart${NCOMP}_volume_singular_l0_allbins_gamma_${GAMMASTR}.npy --config-train config_train_voxelmorph_shell.json --nepochs 1000 --kept-bins "0,1,2,3,4" #--lr 0.0002 --init-weights $1_volumes_allbins_denoised_gamma_0_8_vxm_model_weights.h5
...
sh shellscripts/run_correct_maps.sh ${filepath}
