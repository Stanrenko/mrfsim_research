...
# $3 : Full MRF dictionary e.g.dico_TR2.06_reco2.99.pkl
# $4 : Channel for displacement calculation
...
python scripts/script_recoInVivo_3D_machines.py build_kdata --filename $1.dat --index -4
# # # #
...
python scripts/script_recoInVivo_3D_machines.py coil_compression_bart --filename-kdata $1_kdata.npy --n-comp $NCOMP --filename-cc $2_bart_cc.cfl --calc-sensi False
...
python scripts/script_recoInVivo_3D_machines.py build_volumes_singular_allbins --filename-kdata $1_bart${NCOMP}_kdata.npy --L0 $NSING --dictfile $3 --useGPU False --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --filename-pca $2_bart${NCOMP}_virtualcoils_${NCOMP}.pkl --filename-weights $1_weights.npy --gating-only True
# # # # #python scripts/script_recoInVivo_3D_machines.py build_volumes_singular_allbins --filename-kdata $1_kdata.npy --n-comp $NCOMP --L0 $NSING --dictfile $3 --gating-only True --useGPU False #--filename-b1 $2_b12Dplus1_$NCOMP.npy --filename-pca $2_virtualcoils_$NCOMP.pkl
...
python scripts/script_recoInVivo_3D_machines.py build_volumes_iterative_allbins_registered --index-ref ${REF} --filename-volume $1_bart${NCOMP}_volumes_singular_allbins.npy  --niter $NITER --file-deformation $2_bart${NCOMP}_volumes_allbins_registered_allindex_deformation_map.npy --filename-weights $1_weights.npy --use-wavelet True --lambda-wav 1e-5 --mu 0.1 --kept-bins "0,1,2,3,4" --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --gating-only True --axis 1 #--filename-b1 data/InVivo/3D/patient.003.v19/meas_MID00088_FID70345_raFin_3D_tra_0_8x0_8x3mm_FULL_new_mrf_us2_b12Dplus1_12.npy

#python scripts/script_recoInVivo_3D_machines.py build_volumes_iterative_allbins_registered --index-ref ${REF} --filename-volume $1_bart${NCOMP}_volumes_singular_allbins.npy  --niter $NITER --file-deformation $2_bart${NCOMP}_volumes_allbins_deformation_map.npy --filename-weights $1_weights.npy --use-wavelet True --lambda-wav 1e-5 --mu 0.1 --gating-only True --kept-bins "0,1,2,3,4" --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy  #--filename-b1 data/InVivo/3D/patient.003.v19/meas_MID00088_FID70345_raFin_3D_tra_0_8x0_8x3mm_FULL_new_mrf_us2_b12Dplus1_12.npy

# # # # # # #python scripts/script_recoInVivo_3D_machines.py build_volumes_iterative_allbins_registered --index-ref ${REF} --filename-volume $1_volumes_singular_allbins.npy  --niter $NITER --file-deformation $2_volumes_allbins_registered_allindex_deformation_map.npy --filename-weights $1_weights.npy --lambda-wav 1e-5 --use-wavelet True --mu 0.1 --gating-only True --filename-b1 $1_b12Dplus1_$NCOMP.npy #--filename-b1 data/InVivo/3D/patient.003.v19/meas_MID00088_FID70345_raFin_3D_tra_0_8x0_8x3mm_FULL_new_mrf_us2_b12Dplus1_12.npy
