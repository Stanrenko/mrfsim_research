...
find $1 -type f -name "*_seqParams.pkl" | while read -r line ; do
    ...
    find $1 -type f -name "${file_no_ext}*ip.mha" | while read -r line2 ; do
        ...
        python script_recoInVivo_3D_machines.py convertArrayToImage --filevolume ${filepath}/${filecorrected} --filedico ${filepath}/${filedico} --suffix "_offset" --apply-offset True --reorient False
    ...
    find $1 -type f -name "${file_no_ext}*oop.mha" | while read -r line2 ; do
        echo "Processing $line2"
        # python script_recoInVivo_3D_machines.py getGeometry --filemha ${line2} --filename ${line} --suffix "_adjusted"
        python script_recoInVivo_3D_machines.py convertArrayToImage --filevolume ${line2} --filedico ${filepath}/${filedico} --nifti True
