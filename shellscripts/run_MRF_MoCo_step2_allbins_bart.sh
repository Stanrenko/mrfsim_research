...
# python script_recoInVivo_3D_machines.py build_kdata --filename $1.dat --select-first-rep True
# # # # #
...
#python script_recoInVivo_3D_machines.py build_volumes_singular_allbins --filename-kdata $1_bart${NCOMP}_kdata.npy --L0 $NSING --dictfile $3 --useGPU False --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --filename-pca $2_bart${NCOMP}_virtualcoils_${NCOMP}.pkl --filename-weights $1_weights.npy --gating-only True
...
then
  ...
  VOLUMESSUFFIX=ref${i}_it$(($NITER-1))
fi
echo $VOLUMESSUFFIX
python script_recoInVivo_3D_machines.py build_volumes_iterative_allbins_registered --index-ref ${i} --filename-volume $1_bart${NCOMP}_volumes_singular_allbins.npy  --niter $NITER --file-deformation $2_bart${NCOMP}_volumes_allbins_registered_allindex_deformation_map.npy --filename-weights $1_weights.npy --use-wavelet True --lambda-wav 1e-5 --mu 0.1 --kept-bins "0,1,2,3,4" --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --gating-only True #--filename-b1 data/InVivo/3D/patient.003.v19/meas_MID00088_FID70345_raFin_3D_tra_0_8x0_8x3mm_FULL_new_mrf_us2_b12Dplus1_12.npy
...
# # # # # # # ###exit
# # # # # # # ##
python script_recoInVivo_3D_machines.py build_mask_from_singular_volume --filename-volume $2_bart${NCOMP}_volumes_allbins_registered_allindex.npy --l ${i} --threshold 0.022 --it 1
cp $2_bart${NCOMP}_volumes_allbins_registered_allindex_l${i}_mask.gif /mnt/rmn_files/0_Wip/New/1_Methodological_Developments/1_Methodologie_3T/#9_2021_MR_MyoMap/3_Data_Processed/log_MRF_MoCo

# #Build maps for all bins
echo "######################################################"
echo "Building MRF maps for all iter"
#VOLUMESSUFFIX=ref0_it1
python script_recoInVivo_3D_machines.py build_maps --filename-volume "$1_bart${NCOMP}_volumes_singular_allbins.npy_volumes_allbins_registered_${VOLUMESSUFFIX}.npy" --filename-mask "$2_bart${NCOMP}_volumes_allbins_registered_allindex_l${i}_mask.npy" --filename-b1 $2_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --dictfile $3 --dictfile-light $4 --optimizer-config opt_config_iterative_singular_shell.json --filename $1.dat # --slices "80"
