...
# $2 : Index for parsing header file - default -1


DICTFILE_LOWBODY="./mrf_dict/dico_TR2.12_reco5.0.pkl"
...
# # python scripts/script_recoInVivo_3D_machines.py build_volumes_singular --filename-kdata $1_bart${NCOMP}_kdata.npy --n-comp $NCOMP --L0 $NSING --dictfile $2 --useGPU False --filename-weights $1_us_weights.npy:
# python scripts/script_recoInVivo_3D_machines.py build_volumes_singular --filename-kdata $1_bart${NCOMP}_kdata.npy --n-comp $NCOMP --L0 $NSING --dictfile ${DICTFILE_LOWBODY} --useGPU False --filename-seqParams $1_seqParams.pkl
# # rm $1_kdata.npy
...
# python scripts/script_recoInVivo_3D_machines.py build_mask_from_singular_volume --filename-volume $1_bart${NCOMP}_volumes_singular.npy --l 0 --threshold 0.015 --it 1


#Build maps for all bins
#echo "######################################################"
#echo "Building MRF maps for all bins"
python scripts/script_recoInVivo_3D_machines.py build_maps --filename-volume "$1_bart${NCOMP}_volumes_singular_denoised.npy" --filename-mask "$1_bart${NCOMP}_volumes_singular_denoised_l0_mask.npy" --dico-full-file ${DICTFILE_LOWBODY}
#python scripts/script_recoInVivo_3D_machines.py build_maps --filename-volume "$1_bart${NCOMP}_volumes_singular.npy" --filename-mask "$1_bart${NCOMP}_volumes_singular_l0_mask.npy" --filename-b1 $1_bart${NCOMP}_b12Dplus1_${NCOMP}.npy --dictfile $2 --dictfile-light $3 --optimizer-config opt_config_iterative_singular_shell.json --filename $1.dat
